...
public class DE10D {
	...
	public static void main(String[] args) {
		for (int k = 0; k < 51; k++) {
			//populating the array with individuals for the population using the method initialize_population
			Individual10D[] initial_parents = initialize_population(100);
			double[] temp = new double[100];
			//defining the dimensions
			int D = 10;
			//defining the number of runs till termination
			int generations = 3000*D;
				
			//terminates when criteria is met that is when generations is <= 0
			while (generations > 0) {
				//For loop to reach every individual in the population so they are either replaced if they are not as good as the child/mutated individual
				for (int i = 0; i <= (initial_parents.length - 1); i++) {
					//generating random numbers so we can randomly choose 3 other candidates
					...
					//random candidates other than current candidate/parent being accessed are selected from the population, help out with mutation and crossover
					...
					double parent_fit = initial_parents[i].get_fitness();
					BigDecimal bd = new BigDecimal(parent_fit);
					System.out.println(bd.toString());
					
					//Mutation method either returns the parents or mutated value based on if parents are more fit than mutation or not
					initial_parents[i] = Mutation(initial_parents[i], candidate1, candidate2, candidate3);
					System.out.println(initial_parents[i].get_fitness());
			...
			for (int j = 0; j <= (initial_parents.length - 1); j++) {
				...
				System.out.println(best.get_fitness());
			}
		}
		//tallyData();
	...
	//Initialization method where we assign random genes to each individual/original parents
	...
	public static Individual10D Mutation(Individual10D parent, Individual10D one, Individual10D two, Individual10D three) {
		double F = 0.8; //this is a constant
		//process of mutation where each gene from 2 candidates are sutracted and transformed with the constant as well as a with the third candidate
		...
		//performing crossove...new child will be a combination of genes from parent and mutation
		...
		//Defining the childs fitness ith the following methods, depending on which benchmark function we want to use
	...
	//These functions calculate the fitness value of the candidate as well as sets their fitness value, they all return nothing, they act like set methods essentially
